#!/usr/bin/env python3
"""
Generate ±100‑amplitude basis vectors for each of the 10 model parameters
(9 Zernike terms + M2Z defocus).  Results are written to
data/synthetic_basis_vectors/ as standard a2000 *.npz files.
"""
...
# ------------------------------------------------------------------
# Output location
OUTDIR = Path("data/synthetic_basis_vectors/a2000/")
...
def make_single_basis(param_name: str, amplitude: float):
    """
    Generate a single synthetic data cube with one parameter set to ±100.
    Returns (images, zernikes_array, subref_array, jobname).
    """
    # Build Zernike param dict (all zeros except possibly one)
    param_dict = {name: 0.0 for name in ZERN_NAMES}

    m2z_offset = 0.0  # default
    ...
    jobname = f"{param_name}_{'p' if amplitude>0 else 'm'}{amplitude}"
